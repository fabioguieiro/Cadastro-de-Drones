...
void  menuTrabalho2()  // segunda parte do trabalho pratico
{
    ...
    while (sair != 1)
    {
        ...
        if(op == 1)
        {
            ...
            while(bye != 1)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    printf("Digite o peso do drone que deseja remover:\n");//Remover um drone pelo peso
                    float salva;
                    scanf("%f",&salva);
                    remove_ArvAVL(a, salva);
                    break;
                case 4:
                    printf("Inserção:\n");
                    struct Drone d1;
                    ...
                    printf("Nome do fabricante: ");
                    fflush(stdin);
                    fgets(d1.fabricante,41, stdin);
                    fabricante[strlen(fabricante)-1]='\0';
                    printf("Nome do modelo: ");
                    fflush(stdin);
                    fgets(d1.modelo,21, stdin);
                    modelo[strlen(modelo)-1]='\0';
                    printf("Capacidade de corrente: ");
                    fflush(stdin);
                    scanf("%d",&d1.cap_mAH);
                    printf("Tempo Maximo no ar (formato HH:MM:SS): ");
                    fflush(stdin);
                    fgets(d1.tempoar,10, stdin);
                    tempoar[strlen(tempoar)-1]='\0';
                    insere_ArvAVL(a,d1);
                    break;
                case 5:
                    printf("Em construção\n");//Buscar e exibir um drone pelo peso
                    printf("Digite o peso do drone que deseja encontrar:\n");
                    scanf("%f",&salva);
                    consulta_ArvAVL(a, salva);
                    break;
                case 6:
                    printf("EM construção\n");//Liberar arvore
                    libera_ArvAVL(a);
        ...
        else if(op == 2)
        {
            ...
            fclose(arq);
            fflush(arq);
            printf("Estrutura de Drones.txt carregada com sucesso!\n");
            int bye = 0;
            while (bye != 1)
            {
                printf("Selecione a tarefa que deseja executar:\n");
                ...
                case 4:
                    ...
                    fgets(d1.tempoar,10, stdin);
                    tempoar[strlen(tempoar)-1]='\0';
                    inserirHeap(&meuHeap, d1);
                    break;
                case 5:
                    printf("Qual o peso do Drone que voce deseja buscar?\n");
                    float salva;//Buscar e exibir um drone pelo peso
                    scanf("%f",&salva);
                    imprimirPeso(meuHeap, salva);
                    break;
                case 6:
